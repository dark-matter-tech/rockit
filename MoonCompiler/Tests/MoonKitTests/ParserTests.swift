...
final class ParserTests: XCTestCase {
    ...
    func testImportDeclaration() {
        let file = parseOK("import moon.core.List")
        XCTAssertEqual(file.imports.count, 1)
        XCTAssertEqual(file.imports[0].path, ["moon", "core", "List"])
    }

    func testMultipleImports() {
        let file = parseOK("""
        import moon.core.List
        import moon.http.HttpClient
        """)
        XCTAssertEqual(file.imports.count, 2)
        XCTAssertEqual(file.imports[0].path, ["moon", "core", "List"])
        XCTAssertEqual(file.imports[1].path, ["moon", "http", "HttpClient"])
    }
    ...
    func testHelloMoonFile() {
        // Read the hello.moon example file and verify it parses
        let source = """
