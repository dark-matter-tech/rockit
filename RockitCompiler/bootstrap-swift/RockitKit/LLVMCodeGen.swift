...
public final class LLVMCodeGen {
    ...
    // MARK: - Debug Metadata (DO-178C 6.3.4g)

    /// Counter for debug metadata IDs (starting after !103 used by DICompileUnit/DIFile).
    private var debugMetadataCounter: Int = 200
    /// DISubprogram entries: (name, metadataId, sourceLine).
    private var functionSubprograms: [(name: String, metadataId: Int, line: Int)] = []
    /// Current function's DISubprogram metadata ID.
    private var currentSubprogramId: Int = 0
    /// Deduplication cache for DILocation: "line:col:scopeId" → metadata ID.
    private var locationCache: [String: Int] = [:]
    /// Accumulated debug metadata lines to append at end of module.
    private var debugMetadataLines: [String] = []
    ...
    public func emit(module: MIRModule) -> String {
        ...
        nonEscapingParams = [:]
        debugMetadataCounter = 200
        functionSubprograms = []
        currentSubprogramId = 0
        locationCache = [:]
        debugMetadataLines = []
        ...
        // Module header
        lines.append("; Rockit LLVM IR -- generated by command 0.1.0-alpha (Stage 0)")
        ...
        // DISubprogram metadata on functions enables per-function traceability.
        // DILocation metadata on instructions maps each generated instruction
        // back to its source line — the core of source-to-object tracing.
        let sourceFileName = module.sourceFileName ?? "unknown.rok"
        let baseFile = (sourceFileName as NSString).lastPathComponent
        let dir = (sourceFileName as NSString).deletingLastPathComponent
        lines.append("")
        lines.append("; DO-178C Debug metadata — source-to-object traceability")
        lines.append("!llvm.dbg.cu = !{!100}")
        lines.append("!llvm.module.flags = !{!103}")
        lines.append("!100 = distinct !DICompileUnit(language: DW_LANG_C99, file: !101, producer: \"rockit\", isOptimized: false, emissionKind: LineTablesOnly)")
        lines.append("!101 = !DIFile(filename: \"\(baseFile)\", directory: \"\(dir)\")")
        lines.append("!103 = !{i32 2, !\"Debug Info Version\", i32 3}")

        // DISubroutineType (shared by all functions)
        let subroutineTypeId = debugMetadataCounter
        debugMetadataCounter += 1
        lines.append("!\(subroutineTypeId) = !DISubroutineType(types: !{})")

        // DISubprogram per function
        for sp in functionSubprograms {
            lines.append("!\(sp.metadataId) = distinct !DISubprogram(name: \"\(sp.name)\", scope: !101, file: !101, line: \(sp.line), type: !\(subroutineTypeId), isLocal: true, isDefinition: true, scopeLine: \(sp.line), unit: !100)")
        }

        // DILocation entries
        for metadataLine in debugMetadataLines {
    ...
    private func collectExternals(module: MIRModule) {
        ...
        externalDecls.insert("declare void @rockit_list_grow(ptr)")
        // Turbo: single-char string split returning list of slices
        externalDecls.insert("declare ptr @rockit_string_split_char(ptr, i64)")
        // I/O operations
    ...
    private func emitFunction(_ function: MIRFunction) -> String {
        ...
        // DO-178C 6.3.4g: Allocate DISubprogram for source-to-object traceability
        let subprogramId = debugMetadataCounter
        debugMetadataCounter += 1
        let funcSourceLine = function.sourceMap.entries.values.map { $0.start.line }.min() ?? 0
        functionSubprograms.append((name: funcName, metadataId: subprogramId, line: funcSourceLine))
        currentSubprogramId = subprogramId

        let linkage = funcName == "main" ? "" : "internal "
        lines.append("define \(linkage)\(retType) @\(funcName)(\(params)) nounwind !dbg !\(subprogramId) {")
        ...
        for block in function.blocks {
            ...
            for (instIdx, inst) in block.instructions.enumerated() {
                ...
                // DO-178C 6.3.4g: debug location suffix for this instruction
                let dbg = debugSuffix(block: block.label, index: instIdx)
                ...
                if case .add(let dest, _, let rhs, let type) = inst,
                   ...
                   let targetVar = selfAppendVar[dest] {
                    ...
                    appendWithDbg(emitted, dbg: dbg, to: &lines)
                    continue
                }
    ...
    // MARK: - Debug Location Helper

    /// Get or create a DILocation metadata ID for a source span.
    private func debugLocationId(for span: SourceSpan) -> Int {
        let key = "\(span.start.line):\(span.start.column):\(currentSubprogramId)"
        ...
        debugMetadataLines.append("!\(id) = !DILocation(line: \(span.start.line), column: \(span.start.column), scope: !\(currentSubprogramId))")
        return id
    }
            ...
            for inst in block.instructions {
                ...
                case .call(let d, let function, _):
                    if let d = d {
                        ...
                        // listGet can return any type — do NOT assume int
                    }
    ...
    private func emitCall(dest: String?, function: String, args: [String]) -> [String] {
        ...
        if function == "toInt", let dest = dest, args.count == 1 {
            ...
            if argType == "ptr" {
                ...
                let resultTmp = nextSSA()
                var lines: [String] = []
                lines.append("\(tmp) = load ptr, ptr \(addrOf(arg))")
                lines.append("\(resultTmp) = call i64 @rockit_string_to_int(ptr \(tmp))")
                lines.append(storeToTemp(dest, value: resultTmp, type: "i64"))
                registerTypes[dest] = "i64"
                knownIntTemps.insert(dest)
                return lines
            }
    ...
    /// Turbo: when value is a known integer, skip rockit_retain_value entirely
    /// (avoids is_likely_heap_ptr → vm_read_overwrite Mach syscall per append).
    private func emitNativeListAppend(args: [String]) -> [String] {
        ...
        let isKnownInt = knownIntTemps.contains(args[1])
    ...
    private func emitToStringCall(dest: String?, args: [String]) -> [String] {
        ...
        case "i64":
            if knownIntTemps.contains(arg) {
                // DAL A: known integer — call rockit_int_to_string directly.
                // Bounded WCET, no is_likely_string_ptr syscall, no snprintf.
                trackHeapTemp(dest, kind: .string)
            ...
            // Fallback: polymorphic toString for unknown i64 (may be string pointer in Stage 1)
            return [
                "\(tmp) = load i64, ptr \(addrOf(arg))",
...
extension LLVMCodeGen {
    ...
    public static func compileToNative(
        ...
        // DO-178C / DO-330: Modular pipeline with typed phase contracts.
        // Each phase is independently verifiable with defined Input → Output.
        // Artifacts are captured for audit traceability.
        ...
        let llvmIR: String
        do {
            llvmIR = try pipeline.compileToLLVMIR(source: source)
        } catch let error as CompilerPipelineError {
            print("")
            diagnostics.dump()
            switch error {
            case .safetyViolations(_, let violations):
                for v in violations {
                    print("  \(v.fullDescription)")
                }
            default:
                break
            }
